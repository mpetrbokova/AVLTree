...
public class RangeTree{
    ...
    public boolean hasConflict(Range query){
        if(byStart.isEmpty() && byEnd.isEmpty()) {
            return false;
        }

        if(byStart.findPrevKey(query.end) != null) { // avl tree latest start time 
            Range temp1 = findByStart(byStart.findPrevKey(query.end));
            if(temp1.end > query.start){
                return true;
            
            }
        } 

        if(byEnd.findNextKey(query.start) != null) { // avl tree earliest end time 
            Range temp = findByEnd(byEnd.findNextKey(query.start));
            if(temp.start < query.end){
                return true;
            }
        } 
